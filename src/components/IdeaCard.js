...
export default function IdeaCard({ idea, allowEdit = true }) {
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      // Only write a version entry when the current authenticated user is the owner
      try {
        if (user && idea.ownerUid && user.uid === idea.ownerUid) {
        ...
        console.warn("Failed to write version entry:", verr);
      }
      ...
      {editing ? (
        <div className="space-y-4">
          <input
            ...
            maxLength={120}
            className="w-full mb-3 rounded-md border border-gray-200 bg-white p-3 text-slate-900 outline-none"
          />
          {editTitleError && <p className="mt-1 text-xs text-rose-400">{editTitleError}</p>}
          <textarea
            value={editContent}
            onChange={(e) => setEditContent(e.target.value)}
            className="w-full rounded-2xl border border-gray-200 bg-white p-4 text-slate-900 outline-none focus:ring-2 focus:ring-opacity-60 focus:ring-purple-300"
            rows={4}
          />
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="w-full mt-2">
              ...
              <select
                ...
                className="rounded-md border border-gray-200 bg-white p-2 text-sm text-slate-900 w-full"
              >
                <option>💡 Idea</option>
          ...
          {idea.title ? (
            ...
            {idea.tags && idea.tags.length > 0 && (
              <div className="mb-3 flex flex-wrap gap-2">
                {idea.tags.map((t) => (
                  <span key={t} className="text-xs bg-gray-100 px-2 py-1 rounded-full text-slate-700">
                    {t}
                  </span>
                ))}
              </div>
            )}
            <p className="text-slate-800 mb-4 text-base leading-7">{idea.content}</p>
        </>
      )}
      <MediaPreview media={idea.media} />
      <div className="border-t border-gray-100 pt-4">
        <p className="text-xs text-slate-600">
          <span className="font-medium text-slate-800">Captured:</span> {capturedAt}
        </p>
        {locationLabel && (
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          <span className="soft-pill">{visibility}</span>
            ...
            <>
              ...
              {shareUrl && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="text-sm text-primary underline-offset-2 hover:opacity-90"
                >
                  View link
                </a>
              )}
